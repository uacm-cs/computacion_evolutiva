...
_STOPWORDS = stopwords.words(
    "spanish"
)  # agregar más palabras a esta lista si es necesario
...
def vectorizar_texto(train_data, test_data):
    ...
    vectorizador = TfidfVectorizer(
        ...
        tokenizer=mi_tokenizador,
        ngram_range=(1, N_GRAMS),
        max_features=features_reducidas,
    )
    X_train = vectorizador.fit_transform(train_data)
...
def funcion_fitness(best_individuo, X, Y, kfolds=0):
    ...
    if kfolds == 0:
        ...
        X_train, X_test, Y_train, Y_test = train_test_split(
            X_reducida, Y, test_size=0.2, stratify=Y, random_state=42
        )
...
def evaluar_poblacion_paralelo(poblacion, X, Y, n_jobs=-1):
    ...
    resultados = Parallel(n_jobs=n_jobs - 1, timeout=300, verbose=10)(  # 5min
        delayed(funcion_fitness)(ind, X, Y) for ind in poblacion
    )
...
def algoritmo_genetico(X, Y, tam_poblacion=50, num_generaciones=20, run_paralelo=False):
    ...
    for generacion in tqdm(range(num_generaciones)):
        ...
        print(
            f"Generación {generacion + 1}: Mejor fitness = {best_fitness:.4f}, Fitness promedio = {mean_fitness:.4f}"
        )
...
if __name__ == "__main__":
    ...
    num_generaciones = 5
    paralelo = True
    X_train_data, X_test_data, Y_train_data, Y_test_data = cargar_datos(
        train_file="./data/dataset_polaridad_es_train.json",
    ...
    best_individuo, best_hist, mean_hist = algoritmo_genetico(
        X_train,
        Y_train,
